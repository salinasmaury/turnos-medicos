...
export default function PaginaGestionTurnos({ auth, pacienteEncontrado = null, medicos = [], turnosDelDia = [], flash }) {
    ...
    const especialidadSeleccionada = useMemo(() => {
        ...
        return medicoSeleccionado ? medicoSeleccionado.especialidad.nombre : null;
            ...
            <div className="py-12">
                <div className="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-6">
                    <div className="p-6 bg-white overflow-hidden shadow-sm sm:rounded-lg">
                        <form onSubmit={handleGuardarTurno} className="space-y-6">
                            {/* --- FILA 1 --- */}
                            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                                <div className="md:col-span-1">
                                    ...
                                    />
                                    <InputError message={errors.medico_id} className="mt-2" />
                                </div>
                                <div className="md:col-span-1">
                                    <InputConLabel
                                        label="Especialidad"
                                        id="especialidad"
                                        value={especialidadSeleccionada || 'N/A'}
                                        readOnly
                                ...
                                <div className="md:col-span-1">
                                    ...
                                    {fechaError && <p className="text-sm text-red-600 mt-2">{fechaError}</p>}
                                </div>
                            </div>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 items-end">
                                ...
                                <div className="md:col-span-1">
                                    <InputConLabel
                                        ...
                                        value={pacienteSeleccionado ? `${pacienteSeleccionado.nombre} ${pacienteSeleccionado.apellido}` : 'Ningún paciente seleccionado'}
                                        readOnly
                                        className="bg-gray-100"
                                    />
                                ...
                                <div className="md:col-span-1">
                                    <PrimaryButton className="w-full justify-center" disabled={!sePuedeGuardar || processing}>
                                        {processing ? 'Guardando...' : 'Guardar Turno'}
                                    </PrimaryButton>
                                </div>
                            </div>
                        </form>
                            ...
                            <div className="mt-4 space-y-2">
                                {turnosDelDia.length > 0 ? (
                                    turnosDelDia.map(turno => (
                                        <div key={turno.id} className="p-3 border rounded-md">
                                            <p><strong>Paciente:</strong> {turno.paciente.nombre} {turno.paciente.apellido}</p>
                                        </div>
                                    ))
                                ) : (
                                    <p className="text-gray-500">No hay turnos agendados para este médico en la fecha seleccionada.</p>
                                )}
                            </div>
                        </div>
                    )}
